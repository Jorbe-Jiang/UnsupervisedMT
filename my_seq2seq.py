...
def my_attention_decoder(decoder_inputs, initial_state, attention_states, cell, output_size=None, num_heads=1, loop_function=None, dtype=None, scope=None, initial_state_attention=False):
...
def my_embedding_attention_decoder(decoder_inputs, initial_state, attention_states, cell,
                                num_symbols,
                                embedding_size,
                                num_heads=1,
                                output_size=None,
  ...
  with variable_scope.variable_scope(
    ...
    return my_attention_decoder(emb_inp, initial_state, attention_states, cell, output_size=output_size, num_heads=num_heads, loop_function=loop_function, initial_state_attention=initial_state_attention)

# I need to call
# return encoder_outputs, encoder_state
def my_dec_encoder(decoder_inputs,
               cell,
               num_encoder_symbols,
               embedding_size,
               dtype=None,
               scope=None):
  with variable_scope.variable_scope(
      scope or "embedding_attention_seq2seq", dtype=dtype, reuse=True) as scope:
    dtype = scope.dtype
    # Encoder.
...
def my_inf_encoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    "embedding_inf_network", dtype=dtype) as scope:
    dtype = scope.dtype
    encoder_outputs, encoder_state = rnn.static_rnn(
      encoder_cell, decoder_inputs, dtype=dtype)
  return encoder_outputs, encoder_state
...
# I need to call
# return outputs, state, actions
def my_stack_embedding_attention_seq2seq(encoder_outputs, encoder_state, decoder_inputs, cell, num_decoder_symbols, embedding_size, num_heads=1, output_projection=None, feed_previous=True, dtype=None, scope=None, initial_state_attention=False, epsilon=0.5, mode=None):
  with variable_scope.variable_scope(
      scope or "embedding_attention_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
    ...
    if isinstance(feed_previous, bool):
      return my_embedding_attention_decoder(decoder_inputs, encoder_state, attention_states, cell, num_decoder_symbols, embedding_size, num_heads=num_heads, output_size=output_size, output_projection=output_projection, feed_previous=feed_previous, initial_state_attention=initial_state_attention)
    else:
